...
static void scalarUpdate(kalmanCoreData_t *this, xtensa_matrix_instance_f32 *Hm, float error, float stdMeasNoise)
{
    ...
    for (int i = 0; i < KC_STATE_DIM; i++) {
        tmpNN1d[KC_STATE_DIM * i + i] -= 1;    // KH - I
    }
...
static void scalarUpdateForSweep(kalmanCoreData_t *this, float measuredSweepAngle, float dp, float dx, kalmanCoreStateIdx_t state_p, float stdDev, xtensa_matrix_instance_f32 *R, float distanceToBs, const uint32_t tick)
{
    if (dx != 0) {
        ...
        if (outlierFilterValidateLighthouseSweep(&sweepOutlierFilterState, distanceToBs, angleError, tick)) {
            ...
            vec3d h_b = {0, 0, 0};
            xtensa_matrix_instance_f32 H_B = {3, 1, h_b};
            h_b[KC_STATE_X] = -dp / n;
            h_b[state_p] = dx / n;

            vec3d h_g = {0, 0, 0};
            xtensa_matrix_instance_f32 H_G = {3, 1, h_g};
...
void kalmanCoreUpdateWithSweepAngles(kalmanCoreData_t *this, sweepAngleMeasurement_t *angles, const uint32_t tick)
{
    ...
    xtensa_matrix_instance_f32 basestation_rotation_matrix = {3, 3, (float32_t *)(*angles->baseStationRot)};
    xtensa_matrix_instance_f32 basestation_rotation_matrix_inv = {3, 3, (float32_t *)(*angles->baseStationRotInv)};

    // Rotate the sensor position using the CF roatation matrix, to rotate it to global coordinates
    xtensa_matrix_instance_f32 CF_ROT_MATRIX = {3, 3, (float32_t *)this->R};
    xtensa_matrix_instance_f32 SENSOR_RELATVIVE_POS = {3, 1, *angles->sensorPos};
    vec3d sensor_relative_pos_glob = {0};
    xtensa_matrix_instance_f32 SENSOR_RELATVIVE_POS_GLOB = {3, 1, sensor_relative_pos_glob};
    ...
    xtensa_matrix_instance_f32 vec_pos_diff = {3, 1, position_diff};
    xtensa_matrix_instance_f32 vec_pos_diff_rot = {3, 1, position_diff_rotated};
    ...
    float distanceToBs = xtensa_sqrt(dx * dx + dy * dy + dz * dz);
...
void kalmanCorePredict(kalmanCoreData_t *this, float cmdThrust, Axis3f *acc, Axis3f *gyro, float dt, bool quadIsFlying)
{
    ...
    static xtensa_matrix_instance_f32 Am = { KC_STATE_DIM, KC_STATE_DIM, (float *)A}; // linearized dynamics for covariance update;
